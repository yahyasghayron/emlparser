...
<body>
  ...
  <script>
    class MultiPartParser {
      ...
      getContentType() {
        ...
        return {
          mediaType: "text",
          subType: "plain",
          args: "charset=utf-8; format=flowed"
        };
      }
    ...
    class EmlReader {
      ...
      getAttachments() {
        let attachments = [],
          mixedPart = this.#multipartParser.getPartByContentType('multipart', 'mixed');
    ...
    // Pass the base64-encoded content to JavaScript
    var emlContent = "<?php echo $base64_content; ?>";

    // Decode the base64 content
    var decodedContent = atob(emlContent);
    console.log(decodedContent)
    // Convert the decoded content to ArrayBuffer
    var arrayBuffer = new Uint8Array(decodedContent.length);
    for (var i = 0; i < decodedContent.length; i++) {
      arrayBuffer[i] = decodedContent.charCodeAt(i);
    }


    const emlReader = new EmlReader(arrayBuffer);

    // Display information in HTML
    document.getElementById("subject").innerText = "Subject: " + emlReader.getSubject();
    document.getElementById("from").innerText = "From: " + emlReader.getFrom();
    document.getElementById("cc").innerText = "Cc: " + emlReader.getCc();
    document.getElementById("to").innerText = "To: " + emlReader.getTo();
    document.getElementById("MessageHtml").innerHTML = emlReader.getMessageHtml();
  </script>
